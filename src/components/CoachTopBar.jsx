...
const CoachTopBar = () => {
  ...
  return (
    <header className="sticky top-0 z-40 bg-white/90 backdrop-blur-md border-b border-slate-200/80 px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
      ...
      <div className="flex-1 max-w-md mx-4">
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search student by name, course, or risk..."
      ...
      {/* Right Controls: Notifications, Coach Profile, Logout */}
      <div className="flex items-center gap-3">
        ...
        >
          <div className="w-8 h-8 rounded-full bg-purple-100 border border-purple-200 flex items-center justify-center text-purple-700 font-semibold text-xs">
            <User className="w-4 h-4" />
          </div>
          <div className="hidden lg:flex flex-col text-left">
            <span className="text-xs font-semibold text-slate-800 leading-tight">{user?.name || 'Dr. Aris Thorne'}</span>
            <span className="text-[11px] text-slate-500">Wellness Coach</span>
          </div>
        </div>
