...
-- Module 8: Weighted Scoring Model Table (Calculated per Alarm per User)
CREATE TABLE IF NOT EXISTS "Weighted Scoring Model" (
    ...
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    UNIQUE(user_id, alarm_name)
);

CREATE INDEX IF NOT EXISTS idx_weighted_scoring_user_alarm ON "Weighted Scoring Model"(user_id, alarm_name);
